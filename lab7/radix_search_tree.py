...
class Dict:
    ...
    def checkR(self, h, v, d, path: list):
        if h == 0:
            return self.itemMin
        if v.get() == h.key.get():
            print(v.get(), *path)
            return v
        if v.bits(d, 1) == 0:
            return self.checkR(h.left, v, d - 1, [*path, 'left'])
        else:
            return self.checkR(h.right, v, d - 1, [*path, 'right'])

    def check(self, v):
        v = bitskey(v)
        return self.checkR(self.head, v, maxb - 1, [])
